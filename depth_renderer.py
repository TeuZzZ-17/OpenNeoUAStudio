...
def _affine_coefficients(source, values):
    ...
    offset = value0 - along_x * x0 - along_y * y0
    return along_x, along_y, offset


def projective_texture_coefficients(
    ...
    """Return QTransform's nine coefficients for perspective-correct UVs."""

    if len(source) != 3 or len(screen) != 3 or len(camera) != 3:
        return None
    distances = [
        max(minimum_distance, camera_distance - vertex[2])
        for vertex in camera
    ]
    x_coefficients = _affine_coefficients(
        source,
        [point[0] * distance
         for point, distance in zip(screen, distances)],
    )
    y_coefficients = _affine_coefficients(
        source,
        [point[1] * distance
         for point, distance in zip(screen, distances)],
    )
    w_coefficients = _affine_coefficients(source, distances)
    if (x_coefficients is None or y_coefficients is None
            or w_coefficients is None):
        return None
    x_u, x_v, x_offset = x_coefficients
    y_u, y_v, y_offset = y_coefficients
    w_u, w_v, w_offset = w_coefficients
    return (
        x_u, y_u, w_u,
        x_v, y_v, w_v,
        x_offset, y_offset, w_offset,
    )
